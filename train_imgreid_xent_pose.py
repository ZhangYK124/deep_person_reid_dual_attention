...
def main():
    ...
    trainloader = DataLoader(
        ImageDataset(dataset.train, transform=transform_train, pose_label=dataset.pose_label),
...
def train(epoch, model, criterion, optimizer, trainloader, use_gpu, freeze_bn=False):
    ...
    for batch_idx, (imgs, pids, _, pose_label) in enumerate(trainloader):
        data_time.update(time.time() - end)
        
        if use_gpu:
            imgs, pids, pose_label = imgs.cuda(), pids.cuda(), pose_label.cuda()
        
        outputs, pose_outputs = model(imgs)
        if isinstance(outputs, tuple):
            loss = DeepSupervision(criterion, outputs, pids)
        else:
            mse_criterion = nn.MSELoss()
            # mse_loss = mse_criterion(pose_outputs, pose_label)
            # loss = criterion(outputs, pids) + 1.0 / (epoch + 1.0) * mse_loss
        ...
        # mse_losses.update(mse_loss.item(), pids.size(0))
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            ...
            #       'MSE Loss {mse_loss.val:.4f} ({mse_loss.avg:.4f})'.format(
            #        epoch + 1, batch_idx + 1, len(trainloader), batch_time=batch_time,
            #        data_time=data_time, loss=losses, mse_loss=mse_losses))
...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20], return_distmat=False):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, pids, camids) in enumerate(queryloader):
            ...
            features, pose_outputs, pose_mask = model(imgs)
            batch_time.update(time.time() - end)
    ...
    print("------------------")
    # print('MSE Loss {mse_loss.val:.4f} ({mse_loss.avg:.4f})'.format(mse_loss=mse_losses))
    # print("------------------")
